...
CLI**. Creating an entry and updating an entry's **title or body** work on a **draft-issue** card — the
kind `/ptp:backlog-add` creates and the kind you get by typing a new item straight into the board. A
card **backed by a real issue or pull request** is **`Status`-writable only**: its body has **no
in-board write path**, so its status transitions still work while any edit touching its title,
description or notes — and every edit that settles a run, since those clear the run baseline in the
body — refuses instead, naming what is missing. ptp never writes to the underlying issue or pull
request to work around that.
