...
`codex.model` must be a model id your own Codex account supports. Every Codex call now names a model and effort: each field comes from the Codex lookup chain owned by `ptp-codex-mode` (the command family's `codex.<family>` entry, then `codex.judgment.*`, then `codex.model` / `codex.reasoningEffort`), ending at the built-in `gpt-6-astra--high` when nothing is configured.
...
| `codex.model` | string | chain, else `gpt-6-astra` | Third step of the Codex lookup chain for the model passed as `-m <model>` to `codex exec`; the chain ends at `gpt-6-astra`. |
| `codex.reasoningEffort` | `minimal` \| `low` \| `medium` \| `high` | chain, else `high` | Third step of the Codex lookup chain for the effort passed as `-c model_reasoning_effort=<effort>`; the chain ends at `high`. |
...
| `models.analyze` | model token | `opus.high` | Per-family model for the `analyze` command family (rule: `skills/ptp-run-at-model/references/family-default-target.md`). |
| `models.prompt` | model token | `opus.high` | Per-family model for the `prompt` command family (rule: `skills/ptp-run-at-model/references/family-default-target.md`). |
| `models.brainstorm` | model token | `opus.high` | Per-family model for the `brainstorm` command family (rule: `skills/ptp-run-at-model/references/family-default-target.md`). |
| `models.plan-review` | model token | `opus.high` | Per-family model for the `plan-review` command family (rule: `skills/ptp-run-at-model/references/family-default-target.md`). |
| `models.apply-review` | model token | `opus.high` | Per-family model for the `apply-review` command family (rule: `skills/ptp-run-at-model/references/family-default-target.md`). |
| `codex.analyze` | model token | `gpt-6-astra--high` | Per-family Codex model and effort for the `analyze` command family; first step of the Codex lookup chain (`ptp-codex-mode`). |
| `codex.prompt` | model token | `gpt-6-astra--high` | Per-family Codex model and effort for the `prompt` command family; first step of the Codex lookup chain (`ptp-codex-mode`). |
...
| `/ptp:prompt-write-to-backlog` | Persists the conversation's accumulated `/ptp:prompt` understanding as one new backlog entry (parked in `Backlog`) instead of a `prompt.md`; it writes the entry as `/ptp:backlog-add` does. |
