...
### `backlog.projectOwner` and `backlog.projectNumber`

Two keys naming **which GitHub Projects board the epic backlog lives on**. The backlog **is** that
board — there is no local backlog file — so `/ptp:backlog` reads
...
**No key names the GitHub account ptp acts as**, because there is nothing to name: ptp uses `gh`'s own
active account and never selects, switches, or overrides it — see [The transport
contract](#the-transport-contract).

Both resolve from the same two layered files as every other ptp key — global first, then project
...
`projectNumber` have both resolved — that is the whole rule. An incomplete
configuration produces a **non-silent refusal** rather than a fallback:
ptp never warns-and-continues, never substitutes a different board for the one that resolved, and
never writes a local backlog file in place of the configured board.

**The refusal comes first, before anything is touched.** An incomplete `backlog.*`
configuration is refused **before** any backlog command runs a `gh` command and **before** any writer
(`/ptp:backlog-add`, `/ptp:backlog-edit`, `/ptp:backlog-run`, `/ptp:backlog-continue`) cuts a feature
branch — so a repository with no board configured never gets a stray branch out of a backlog command.
Only `backlog.projectOwner` and `backlog.projectNumber` can ever be reported **missing**.

**ptp reaches the board through the `gh` CLI.** `gh` must be installed, an account must be
authenticated, and that account's token must carry `read:project` (to read the board) or `project` (to
write it). ptp acts as **`gh`'s own active account** and never selects, switches, or overrides it — it
sets no `GH_HOST`, `GH_TOKEN`, or `GITHUB_TOKEN`, and runs no `gh auth` command that changes a
credential. Every board identity ptp renders names the account it used. A failed preflight names the
**stage** that failed and the exact command that repairs it — two of which are `gh`'s own words,
`gh auth refresh -s read:project` and `gh auth refresh -s project`.

#### The transport contract

`ptp-github-projects-gh` is the single source of truth for reaching the board, and it is pure prose —
it reads no file on its own and changes nothing.

- **The acting identity has exactly one source, and there is no second:** `gh`'s own resolved active
  account for the resolved host. No configuration key supplies it, no contract constant supplies it,
  no environment variable supplies it, and nothing merely visible in the session supplies it. ptp
  **reads and discloses** that identity rather than choosing it, and never runs `gh auth login`,
  `gh auth switch`, or `gh auth refresh` — those are yours to run.
- **Four ordered preflight stages.** **binary** (is `gh` on PATH), **authentication** (is an account
  authenticated and healthy for the host), **token scope** (which Projects scopes the token carries),
  and **board reachability** (does the configured board come back with its id). The ladder stops at the
  first failing stage, so a logged-out user is told about authentication rather than about an
  unreachable board.
- **Three verdicts.** `ready` (the token carries `project` — everything proceeds), `read-only` (the
  token carries `read:project` but not `project`, or its scopes could not be established — reads
  proceed, writers stop), and `unavailable` (any stage failed — every operation reaching the store
  stops).
- **A failed preflight never silently proceeds.** It STOPs with a fixed, greppable message naming the
  failing stage, the `gh` version, the account and host `gh` resolved, the configured board with the
  layer each value came from, the likely cause, and a repair line — which names `/ptp:config` only
  where a configuration change can actually help, and otherwise names the exact `gh` command to run.
  There is no `--force`.
...
naming the missing keys, before it runs the preflight or runs any `gh` command. The four writers —
...
**One-time board setup.** Create the board (or reuse one), then add **one** custom field to it — that
is the whole setup, and ptp does not create it for you:

| Field | Type | Notes |
|---|---|---|
| `Status` | **single select** | Needs an option for **every** entry status value. The **workflow columns** — the ones a card is actually moved through — are, **in this order**: `Backlog` → `Ready` → `In Progress` → `In Review` → `Done`. `Blocked` and `Cancelled` are required as options too, but as options **ptp writes** when a run halts or an epic is cancelled — not columns you drag a card through by hand. The names below are the **defaults**, echoed from the `ptp-backlog` skill: `backlog` (or `Backlog`), `ready` (or `Ready`), `in-progress` (or `In Progress`), `in-review` (or `In Review`), `done` (or `Done`), `blocked` (or `Blocked`), and `cancelled` (or `Cancelled` / `Canceled`). **A board whose columns are already named something else does not rename them** — set [`backlog.statusOptions`](#backlogstatusoptions) instead, per status row. **The vertical order of the cards inside `Ready` is the run order:** `/ptp:backlog-run` takes the top card first and works down, so re-prioritising is dragging a card up the column. |

Two consequences follow, and both matter: **every card on the board is a backlog entry — do not park
unrelated cards on it**; and a card with **no `Status`** is reported as a defect that **withholds the
ready set** until it is set. A `Backlog ID` field left over from an earlier ptp is simply ignored —
never read, never written, never removed — and may be deleted by hand.
...
written, and never removed. A board missing the required field is reported fatally and actionably —
ptp never creates a field, an option, an item, or a project, and it never **reorders** the field or its
options: the column order above is an instruction to you, not something ptp establishes or enforces.

**The order that *is* ptp's is the other one** — the **vertical order of the cards inside `Ready`**, from
the `Status` row above, not the left-to-right column sequence the paragraph above just disclaimed. That
order is the
**canonical order the `ptp-backlog` skill defines** — this material echoes it and adds
nothing to it. ptp **reads** the card order and never writes it: it still creates no project, no field, no
option and no card, and reorders no field, no option and **no card**. One caveat worth knowing: ptp reads
the **project's** card order, which is what a board view shows when the view applies no sort of its own; a
view you have explicitly sorted by some other field displays an order ptp does not see and does not
follow.

The **`ptp-backlog` skill owns the status contract** — the entry status values, the default option table
above, and which names map onto which status — and is authoritative. The table on this page echoes it in
user-facing terms; where the two disagree, the skill wins.

All three `backlog.*` keys — the two above and [`backlog.statusOptions`](#backlogstatusoptions)
below — are settable via `/ptp:config` (see below).

#### Migrating a board built before this release

If you built your board under the previous instructions — a `Status` field carrying `Todo`,
`In Progress`, `Done`, `Blocked`, `Cancelled` — read this. The status contract changed; your board did
not.

**ptp still creates and reorders nothing.** No project, no custom field, no `Status` option, no card, and
no reordering of the field or its options — that was true before this release and is unchanged by it.
Nothing on your board was touched. The migration below is a **board edit or a configuration key that you
perform**; no ptp command performs it for you.

**Your board keeps working only insofar as its option names still match** the current default table — or,
if you have set [`backlog.statusOptions`](#backlogstatusoptions), the table your configuration resolves
to, which the last block of this subsection covers. The
names that still match under the defaults, and are therefore unaffected, are `in-progress` and
`In Progress`; `done` and
`Done`; `blocked` and `Blocked`; and `cancelled`, `Cancelled`, and `Canceled` — both the lowercase and
the display spellings, exactly as before. What is new is that `Backlog`, `Ready`, and `In Review` are now
default names too, for the new `backlog`, `ready`, and `in-review` statuses.

**`Todo` no longer maps to any status by default, and `pending` is no longer a status value at all.**
Every card sitting in a `Todo` column is therefore carrying an option ptp no longer recognizes: those
entries are reported as defective on their status, and the **ready set is withheld** until they are
settled, so `/ptp:backlog-run` takes nothing meanwhile. The exact reporting is the `ptp-backlog` skill's
problem vocabulary, which is authoritative — this page names the consequence, not the code.

**Two remedies, and they are the only two.**

1. **Rename or add the board's columns** to the current names — `Backlog`, `Ready`, `In Progress`,
   `In Review`, `Done`, plus `Blocked` and `Cancelled`.
2. **Set [`backlog.statusOptions`](#backlogstatusoptions)** to restore your previous spellings — for
   example putting `Todo` back on a status row — with no board rename at all.

**Renaming alone is not a full restoration**, and this is the part worth reading twice. The
`Backlog` / `Ready` distinction is a **workflow change, not a rename**: `Backlog` is the parked column
and `Ready` is the runnable one, and `/ptp:backlog-run` takes only the `Ready` entries. So if you rename
`Todo` → `Backlog`, every epic that used to be picked up will sit in `Backlog` and **will not be run**
until you move it to `Ready` — which is the point of the split. If you want the previous behavior back in
one step instead, map your old column onto the **runnable** status rather than the parked one:
`"ready": "Todo"` in `backlog.statusOptions`.

**If you already use `backlog.statusOptions`**, three things are true, and the first alone would mislead:

- A row keyed on a status the widened set **still recognizes** — `in-progress`, `done`, `blocked`,
  `cancelled` — is **unaffected** by the default-table change. Your mapping applies exactly as before.
- A row keyed on a status that **no longer exists** — a `"pending"` row written under the previous
  instructions — is now an **unrecognized key**, and the forgiving reader **ignores** it. That mapping
  silently stops applying, and the row must be re-keyed. If you want that column to keep being picked up
  by `/ptp:backlog-run`, re-key it to `"ready"`; if it was a parking column, re-key it to `"backlog"`.
- Your board may carry **no option at all** for the new statuses (`ready`, `in-review`, and possibly
  `backlog`). That surfaces as `/ptp:backlog`'s existing missing-option **advisory** — information, not a
  new failure and not a gate — and a write to a status your board offers no option for is refused rather
  than guessed.

### `backlog.statusOptions`

Your board's `Status` column names are a **board-shaped fact**, so they are configuration rather than a
ptp constant. `backlog.statusOptions` lets a board running `Icebox / Queued / Doing / Reviewing /
Shipped / Blocked / Dropped` be used as the epic backlog **without renaming a single column**.

**The key and its shape.** JSON path `["backlog","statusOptions"]`, sharing the same `backlog` parent as
the three keys above. Its value is an **object** whose recognized keys are the seven entry status values
— `backlog`, `ready`, `in-progress`, `in-review`, `done`, `blocked`, `cancelled` — and whose value per
key is either a **string** or an **array of strings** naming the board `Status` option name(s) that carry
that status:
...
A key outside those seven is **ignored** — never an error, and it never adds an eighth status.

**The default is unset**, which means the built-in option table in full: `backlog` from `backlog` or
`Backlog`; `ready` from `ready` or `Ready`; `in-progress` from `in-progress` or `In Progress`;
`in-review` from `in-review` or `In Review`; `done` from `done` or `Done`; `blocked` from `blocked` or
`Blocked`; `cancelled` from `cancelled`, `Cancelled`, or `Canceled`. With the key unset, every board
item's status is interpreted exactly as the built-in table says.

**Precedence is per status key.** Global first, then project — and within this key, **status key by
status key**. Configuring only `done` leaves the other six rows at their defaults. A configured row
**replaces** its default row rather than extending it, so configuring `backlog: "Icebox"` means
`Backlog` no longer reads as `backlog` — which is the point: on a board whose `Backlog` column is where
**run-ready** epics live, you need to be able to stop it meaning `backlog` so you can map it onto `ready`
instead (`"ready": "Backlog"`). If you want both spellings, list both.

**The reader is forgiving, in two distinct ways.**

- A layer that is **missing, unreadable, unparseable, has a non-object root, a non-object `backlog`, or
  a non-object `statusOptions`** supplies **no status key at all** — it is skipped for **all seven**.
- An **invalid value for one status key** (`""`, `[]`, `[""]`, `3`, `true`, `null`, an array with a
  non-string element) is ignored for **that key only**, and never discards the same layer's valid
  siblings.

Either way the value falls back to the prior valid layer, and ultimately to **that row's own default**.
Resolution never throws and never STOPs over a config typo. Names are **trimmed** and
case-insensitively de-duplicated within a row, and matching an option name stays **case-insensitive and
whitespace-trimmed** — configuration changes *which* names are in the table, never *how* a name is
matched.

**Two refusals worth knowing about**, described here in user-facing terms; the owning skills are
authoritative and this section adds no bound they do not state.

- **A colliding table refuses before any board call.** If one option name ends up on two rows of the
  resolved table — for example setting `ready: "Done"` while `done` keeps its default row —
  `/ptp:backlog` refuses non-silently, naming `backlog.statusOptions`, the colliding name, and every
  status claiming it. It is never resolved by picking one. The rule lives in the `ptp-backlog` skill.
- **A write refuses when your board carries no option for the status.** When ptp commits a status it
  writes the **first name in that status's resolved row that your board actually offers** — so under the
  defaults a board offering `In Progress` and not `in-progress` gets `In Progress`. If the board offers **none** of the
  row's names (or two options whose names differ only in case), the operation is refused **before
  anything is created or written**, naming the row's accepted names, your board's actual options, and
  both repairs. ptp creates no `Status` option, ever. The rule lives in the `ptp-backlog-write` skill.

`/ptp:backlog` additionally emits an **advisory note** naming any status your board can carry no option
for. It raises no problem code, withholds nothing, and changes no verdict — it is information, not a
gate.

**Settable via `/ptp:config`** (see below): pick the parameter, then pick which status row to set, then
type the names comma-separated. Two limitations are worth stating up front: **returning a row to its
default requires deleting that key by hand** (the editor writes values and never removes them —
re-typing the default names is equivalent in effect), and an option name **containing a comma** must be
hand-edited into the config file, which the forgiving reader accepts.
...
2. **Parameter** — one of the **fifteen** registered parameters: `codex.mode` ("Use Codex for
   ...
   `parallel.maxConcurrency` ("Max parallel fan-out members"),
   `backlog.projectOwner` ("Backlog project owner"), `backlog.projectNumber` ("Backlog
   project number"), or `backlog.statusOptions` ("Backlog status option names"); the menu grows as the
   registry grows.
3. **Value** — select from the valid enum values with one-line descriptions (`codex.mode`,
   ...
   (`codex.model`, `telemetry.root` — validated to stay inside the repository —
   and `backlog.projectOwner` — validated as a login rather than a board URL), enter an integer
   (`review.maxIterations`, `telemetry.retentionDays`, `backlog.projectNumber`, `telemetry.port` —
   validated as a TCP port in `1..65535` — and `parallel.maxConcurrency` — validated as `1..10`), or,
   for the one **map** parameter (`backlog.statusOptions`), pick which of the seven status rows to set
   from a **member menu** and then type that row's option names as a **comma-separated** list (a
   one-name row is stored as a JSON string, a multi-name row as a JSON array; an empty element or a row
   that would collide with another row is rejected and re-prompted).
...
**`/ptp:backlog`** — read-only view of the epic backlog, which lives on a **GitHub Projects v2 board** (configured by `backlog.projectOwner` / `backlog.projectNumber`). Shows a header naming the board and carrying the capability-preflight verdict, the entries table with each entry's status, the computed ready set in run order (or the stated reason it is withheld), stale-`in-progress` flags, provisional (`folder-diff-unconfirmed`) change-epic links alongside undispositioned attribution warnings, a scope note (archived entries, ignored metadata keys, degraded scope, and a legacy-file line), and any validation problems. The header also names **where each value came from** — its **provenance**: which configuration layer supplied the owner and the project number (`global config` or `project config`, by role, never as a filesystem path), and that the acting GitHub account and host were **resolved by `gh`** rather than by anything you configured; a value that no configuration layer and no source supplied is **not rendered at all**, so the header can never name a board nothing configured, and where the preflight stopped before authentication passed the header discloses that the identity was not established and names the stage. Its **first action** is the configuration gate: with `backlog.projectOwner` or `backlog.projectNumber` unset it refuses non-silently, naming the missing keys, with no header and **no `gh` command run**. It takes no argument, runs no branch guard, and **creates nothing** — no project, no custom field, no `Status` option, no card, no version marker, and no file. A board it cannot read is **never** rendered as an empty backlog.

**`/ptp:backlog-add "<free-text description of the epic to add>" [model:<model>.<effort>]`** — adds **one** epic to the backlog from a free-text request: writes a single `backlog` entry whose description keeps the request text substantively verbatim, and **modifies no other entry**. The entry lands in the **parked** column (`Backlog`), so it is **not** taken by the next `/ptp:backlog-run` until someone moves it to `Ready`. The entry's id is the board item's own node id — nothing is minted. The report names the entry it created, with its id, title, and status. Persistence runs `ptp-backlog-write`'s ordered write sequence — the create carrying title and body, then the `status: backlog` commit last, **two dispatches** — with both re-reads and a full write journal, so a partial failure is inert, exactly reported, and never compensated. It carries **exactly one** up-front refusal, naming its own cause under an unusable backlog store (an incomplete configuration, a preflight verdict that does not admit a write, or a writer-ineligible store). Autonomous — it asks no clarifying questions.

**`/ptp:backlog-edit <node-id> "<what to change>" [model:<model>.<effort>]`** — edits **exactly one** backlog entry from a free-text instruction: its `title`/`description`/`notes` and its `status` — but only along the rows of the transition table, so every runner-only row (`ready → in-progress`, `in-progress → in-review`, and the runner's own direct `in-progress → blocked` halt, which the user reaches only as a recovery disposition), every row absent from the table, and every no-op write are refused — a runner-only row naming the row and the command that does perform it. Resetting a `blocked` entry or cancelling one retains the prior attempt's `changeEpics` and requires an acknowledgement that its unconverged slices were resolved. It is also the **recovery** command: an entry left in a stale `in-progress` state — un-reconciled from a crashed `/ptp:backlog-run` only if no backlog run is currently live, since a live run looks identical on disk — is reconciled against its `runBaseline`, **gated** while it holds any `changeEpics` id or any undispositioned attribution warning, and settled only through a disposition the availability table actually offers (**claim** → `blocked`, **disown** → `ready` and withheld once a confirmed id exists, **rerun anyway** → `ready`, plus per-prefix **promote**/**dismiss**) — clearing `runBaseline` in the same write **group**, and **never** yielding `done`. Every mutation of one invocation lands in a single write group dispatched through `ptp-backlog-write`'s ordered sequence, with every `runBaseline` clear a payload write preceding the `status` commit; a **refusal** still leaves the store untouched, while a **failure** mid-sequence is named exactly by the journal and its verdict rather than claimed to be byte-clean. It carries **exactly one** up-front refusal, naming its own cause under an unusable backlog store. Autonomous — an ambiguous instruction is refused with the offered dispositions printed, never asked about.

**`/ptp:backlog-run [rounds:{count}]`** — runs ready backlog epics through **`/ptp:full`** in the backlog's canonical order, one at a time, **five per invocation by default** (`rounds:{count}` overrides it for that invocation only; the body is a positive integer and nothing is persisted). It takes no selector — which epics run is the ready set's job, and that set is the **`Ready`** entries, so an epic parked in `Backlog` is never taken. It recomputes the ready set after **every** epic from a fresh validated read, so a mid-run hand edit or store defect is seen, marks each epic `in-progress` with a `runBaseline` snapshot before its `/ptp:full` begins, records the change ids that run produced back into the entry, leaves a **converged** epic at `in-review` — converged but not yet archived — and **halts the whole run** on the first epic whose `/ptp:full` does not converge, marking it `blocked` and taking no further epic. It writes `done` **nowhere**: it still never archives or deploys, so `/ptp:backlog-continue` is what carries an epic from `in-review` to `done`. Each of its three write points is **one write group** dispatched through `ptp-backlog-write`'s ordered sequence, and a group that does not complete halts the run under its own terminal state rather than being reported as a convergence halt. It carries **exactly one** up-front refusal, naming its own cause under an unusable backlog store, and the capability preflight is an **aborting precondition ahead of the branch guard**. Every epic lands on **one** feature branch, uncommitted and unarchived: the command **never commits, pushes, merges, archives, or deploys**, and announces that blast radius — **six** items, the sixth naming that backlog status writes land on a shared board, immediately, outside git, and are not undone by discarding the branch, and that on an entry backed by a real issue or pull request the run's body-carried writes also edit that issue's or pull request's own body — a repository artifact outside the board — before the first epic runs. It classifies its end into **five** loop-terminal states and reports **four** backlog-level buckets.

**`/ptp:backlog-continue [what went wrong during the manual check]`** — the settle path for the epic `/ptp:backlog-run` left `blocked` or `in-review`, and the **only** command that can reach `done` at all — from **either** of those two states. It takes **no selector** — it finds the entry itself, the candidate being one that is `blocked` **or** `in-review` with a non-empty `changeEpics`, with `blocked` taking precedence and **refusing, naming every candidate**, when more than one `blocked` entry qualifies rather than guessing (the backlog persists no attempt boundary, so there is no field on which "the most recent halt" could be computed) — and refusing likewise when **no** entry of either status qualifies; several `in-review` candidates are resolved to the canonical-order head rather than refused. **Bare** means *"I performed the manual verification, it's fine"*: for every change-epic prefix the entry records, it flips the remaining `- [ ]` boxes to `- [x]` (inventing, removing, and reordering nothing), **re-runs** `openspec validate --strict` plus the project's build and test suites so a stale automated failure is a refusal rather than something waved through, drives **`/ptp:review-full`** to its convergence gate and then **`/ptp:archive`** under its own unweakened gates — and only once **every** prefix has settled performs the single `blocked → done` **or** `in-review → done` write group, per the target's own source status (`ptp-backlog`'s guard 3: retain `changeEpics` by planning no row for it, clear `runBaseline` as the payload, commit `done` last, and send no `updatedAt` — the board maintains it). Any stall, and any write group that does not complete, leaves the entry in the status it started from with `changeEpics` intact, and the report says which prefixes finished and **never** reports the epic as finished. It carries **exactly one** up-front refusal, naming its own cause under an unusable backlog store, with the capability preflight evaluated **before** it selects a target. **With free text** it means *"I found problems"*: one scoped fix pass runs against the same change carrying your text verbatim as its brief and `agents/ptp-apply.md`'s hard rules (TDD, **no invented tasks**), no manual-only box is ever re-checked on your behalf, and **no status transition, no review, and no archive** happen — the epic stays in whatever status it had, waiting on another manual check. It is deliberately **UNWRAPPED** (so `/ptp:review-full`'s and `/ptp:archive`'s own spawns stay at one nesting level and `/ptp:archive`'s interactive confirmations stay reachable), never commits/pushes/merges/deploys, and **never chains into `/ptp:backlog-run`** — it points at it instead.

**The board's write surface, for all four writers.** The backlog board is written through the **`gh`
CLI**. **Creating** an entry produces a **draft-issue** card — the kind `/ptp:backlog-add` creates and
the kind you get by typing a new item straight into the board. **Updating** an entry's **title or body**
works on **every** card, whatever backs it. On a card **backed by a real issue or pull request**, that
update edits **that issue's or pull request's own title or body** — a title edit changes its title, and an
edit touching **any body-carried field** changes its body: the description, the notes, the run baseline a
run settles, and the change epics and attribution warnings a run records all live in the card's body.
That is a **repository artifact**: the edit is visible outside the board, to everyone who can see the
issue or pull request, and discarding a feature branch does not undo it.
**`Status`** writes work everywhere, on every card.

**What your GitHub token has to allow.** Projects v2 access is scoped separately from repository
access: **`read:project`** is needed to *read* the board and the stronger **`project`** scope to *write*
it, so a token that renders `/ptp:backlog` fine can still leave every writer refusing. **ptp never runs
`gh auth refresh`** — granting the scope is a user action, taken deliberately and outside ptp.
...
| `ptp-backlog` | Owns the epic backlog **board** contract — the store being one GitHub Projects v2 board per repository (resolved through `ptp-github-projects-gh`, with no local backlog file, no second store and no fallback), the rule that **every board item is an entry**, the ten-field entry model and its tolerant read, the mapping of those ten slots onto **five** board carriers (the one required custom field `Status`, the card's title and body, and the board's own stamps) with the status option table, the sentinel-fenced metadata block and its malformed-body boundaries, and unknown-key preservation in both scopes, the `ptp-backlog-version:` marker and its gate (an absent marker reads as v1), the read-only read protocol with its configuration-completeness-then-preflight precondition, its returned handle table and its degraded scope, the **node-id identity rule** under which nothing is allocated, minted or written, the validator and its fixed **four-code** problem vocabulary with the fatal/structural split and a writer-eligibility rule that refuses past **fatal problems only**, with the distinct `unreachable-store` outcome and the honest-failure rule, and the ready-set definition — the `ready` entries in the board's **item-position order**, which is the column read top-first wherever a view adds no sort of its own — with its order deterministic over the produced document. A pure prose contract in the single-source-of-truth pattern of `ptp-branch-guard` (branch safety), `ptp-codex-mode` (the reviewer gate), `ptp-agent-roles` (role resolution), and `ptp-parallel-fanout` (fan-out safety) — it reads nothing on its own, writes nothing, and edits nothing. Also owns the **status transition table** — eleven rows, each naming its performer and cited by its from → to pair, never by number — with its three guards (the gated `blocked → ready` reset that retains the prior attempt's `changeEpics`, the `any → cancelled` guard, and **guard 3**'s two resume rows `blocked → done` and `in-review → done`, available only as the direct, same-invocation result of `/ptp:backlog-continue`'s own review-full → archive sequence settling every recorded prefix — never as a standalone disposition, so recovery still never yields `done`), the `in-progress → in-review` convergence row that makes `in-review` the resting state of a converged-but-unarchived epic and leaves `/ptp:backlog-run` writing `done` nowhere, and the **recovery-and-reconciliation machinery** every writer that settles a stale `in-progress` entry runs: the stale definition and its deliberately conditional wording, the single change-prefix-set definition both the `runBaseline` snapshot and the reconciliation diff cite, the additive-only reconciliation, the gate, the availability table and the disposition outcomes (`claim` / `disown` / `rerun anyway`, and per-prefix `promote` / `dismiss`) with their combination rules, the every-settling-edit-clears-`runBaseline` rule, and the never-yields-`done` rule. The contract was defined over a local file by `0036_01`, which ships no writer; the transitions and recovery machinery by `0036_03` alongside `/ptp:backlog-edit`, the runner in `0036_04`; the store became a GitHub Projects board in `0042_03`, which ships the read half and leaves every writer refusing. |
| `ptp-backlog-write` | Owns **how a backlog write is dispatched onto the board and what a partial failure means** — the deterministic **ordered write sequence** (existence → payload → the single `status` commit) with the one justification for `status`-last stated there and nowhere else; the **status-commit invariant** that replaces atomicity, with its **backstop refusal** of any operation writing `status` on more than one entry; the **field-is-the-unit-of-planning / carrier-is-the-unit-of-dispatch** rule and the **compose-from-a-fresh-read-of-the-carrier** rule that keeps it from losing an update; the **two re-read rules** — the pre-dispatch snapshot every decision binds to, and the per-field pre-write check over exactly two field categories with **deliberately no third** — plus the **degraded-scope dispositions** derived from what the read path withholds; the **write journal** (one row per planned field, six exhaustive outcomes, six terminal verdicts); **fail-stop** with **no compensating writes** on three independent grounds; the **ambiguous-create scan** read against the snapshot's match set and the **single orphan repair shape**; and the **`runBaseline`-clear dispatch decision** with its accepted residual, its two-layer detection rule and its four-part report obligation. A pure prose contract: it states obligations, performs none, and reads, writes and edits nothing. Delegates the schema, validator, writer eligibility, transition table, guards and recovery machinery to `ptp-backlog`; the field mapping and the read path to `0042_03`'s read contract; and transport, the acting identity, the `gh` surface and the capability preflight to `ptp-github-projects-gh`. |
...
| `ptp-backlog-continue` | Owns the halted-epic **resume** path behind `/ptp:backlog-continue` — the target-selection rule (candidate = `blocked` **or** `in-review` with a non-empty `changeEpics`, `blocked` taking precedence; exactly one proceeds, zero or many `blocked` candidates **refuse** naming what was found, with no invented tie-break because the store persists no attempt boundary, while several `in-review` candidates resolve to the canonical-order head), the **bare/issue-text split** as the user-supplied signal, the **unwrapped outer-session execution contract** that keeps `/ptp:review-full`'s, `/ptp:archive`'s, and the fix pass's spawns at one nesting level (and `/ptp:archive`'s interactive confirmations reachable), the **bare flow** (per-prefix folder lookup with an absent folder skipped rather than failed, checkbox sign-off, re-verification before that sign-off is trusted, the review-full convergence gate including the `ptp-codex-mode` mode-skip success state, archive under its own unweakened gates, then the single `blocked → done` or `in-review → done` write once **every** prefix has settled — with the sign-off documented as durable and the retry shape idempotent), the **issue-text fix pass** that transitions nothing and re-checks no manual-only box, and the four terminal report shapes. Delegates the store contract and the transition table's `blocked → done` / `in-review → done` rows under guard 3 to `ptp-backlog`, branch safety to `ptp-branch-guard`, the reviewer gate to `ptp-codex-mode`, spawn-and-relay to `ptp-run-at-model`, and review/archival to `/ptp:review-full` and `/ptp:archive`. |
| `ptp-github-projects-gh` | Owns the GitHub-Projects backlog **transport contract** and the `backlog.*` **configuration schema**, in eight contract sections: the layered, forgiving per-key resolution of `backlog.projectOwner`, `backlog.projectNumber`, and `backlog.statusOptions` — the last a **map** whose members sit at the schema's first three-level path, resolved **per status key** and published as validated overrides on the verdict, with the default table, the merge and the collision rule left to `ptp-backlog` — with its once-on-the-combination completeness verdict; **the acting identity**, which has exactly one source (`gh`'s own active account for the resolved host) and is read and disclosed rather than chosen; the **enumerated, never-probed `gh` surface**, a subcommand's existence being a property of the binary rather than of the session; the once-per-invocation **four-stage capability preflight** (binary → authentication → token scope → board reachability) and its three verdicts (`ready` / `read-only` / `unavailable`); the **eleven-field preflight record** every consumer reads; the **`archiveReachable`** fact and its three literals; the fixed non-silent **six-label STOP message** and its self-naming repair ladder; and the **shell-invocation contract** every concrete `gh` call is built from. A pure prose contract in the single-source-of-truth pattern of `ptp-branch-guard` (branch safety), `ptp-codex-mode` (the reviewer gate), `ptp-agent-roles` (role resolution), `ptp-parallel-fanout` (fan-out safety), and `ptp-backlog` (the backlog board contract): it reads no file on its own, writes nothing, runs no git, and edits nothing. Defined by `0047_05`, which retires the MCP transport contract it replaces; consumed by `0047_06` (the read path) and `0047_07` (the write path). |
...
Epic backlog
  ...
  → /ptp:backlog-add "<epic request>"                # autonomous: one new entry, lands parked in Backlog, no other entry touched
  → /ptp:backlog-edit <node-id> "<what to change>"   # fields, transition table, crash recovery
  → /ptp:backlog-run [rounds:{count}]                # the Ready epics through /ptp:full, 5 per run, halts on non-convergence
  → /ptp:backlog-continue ["<what went wrong>"]      # finish the halted or in-review epic (bare) or one scoped fix pass (free text)
...
| Version | Changes |
|---------|---------|
| **0.2.33** | **A code review now leaves a durable, fingerprinted convergence marker, so `/ptp:backlog-continue` can skip a redundant `/ptp:review-full` instead of always re-running it.** Every `kind = code` review loop (`/ptp:review-full`, `/ptp:review-loop`, `/ptp:codex-review-loop`, and the `ptp-review` workflow agent) now writes `openspec/changes/<id>/reviews/code.json` on every terminal outcome except a pre-run STOP or a `FIX_TARGET_ESCALATION` dispatch signal — mirroring the existing `reviews/plan.json` marker's atomic write-temp-then-rename protocol, but keyed by a `fingerprint` covering the base branch, merge base, working-tree diff, untracked files, **and** direct content hashes of `tasks.md`/`proposal.md`/`design.md`/`specs/**/spec.md` (`openspec/` is gitignored in this repo, so no git-derived signal alone could see a later checkbox flip). `ptp-backlog-continue`'s bare flow now checks that marker, after its own checkbox sign-off and re-verification, and skips `/ptp:review-full` only when **six** conjunctive conditions hold — the marker parses as `kind:"code"` with a recognized fingerprint version, the recomputed fingerprint matches exactly, the recorded `minSeverity` is at least as strict as the one currently configured, and (new condition 6) the marker's `reviewers` set is sufficient for **the reviewer set a review invoked right now would actually run** — resolved fresh via `ptp-agent-roles` + `ptp-codex-mode` at check time, never cached from or inherited from the marker itself, so a standalone single-reviewer `/ptp:review-loop`/`/ptp:codex-review-loop` marker can never authorize skipping a review that `codex.mode` currently makes two-phase. Any other outcome is fail-closed to today's unconditional review-full; `/ptp:archive`'s review-clean confirmation is unchanged and still performed, now carrying the verified marker as its evidence instead of nothing. **No migration** — existing archived changes without a marker simply always re-review, exactly as before this change. (0052_01) |
| **0.2.29** | **The backlog write path stops refusing issue- and pull-request-backed entries — every board item is now fully writable, not just draft cards.** The 0.2.28 residual limit ("an issue- or PR-backed entry's title and body have no in-board write path, so those writes refuse per entry") is gone: `ptp-github-projects-gh` admits a `gh api graphql` content mutation (`updateIssue` / `updatePullRequest`, addressed by the content's own node id) alongside the existing draft-issue `item-edit` route, with a new `contentScope` preflight field disclosing whether the token's granted scope covers a repository write (`yes`/`no`/`indeterminate` — a disclosure, not a fourth verdict; a repo write can still 403 at runtime). The read path's handle table generalizes `draftIssueId` into `contentNodeId`, published for every content type the transport exposes, and the archived-item raw read now returns that id for `Issue` and `PullRequest` content too, not just `DraftIssue`. `ptp-backlog-write`'s dispatch table gains a per-content-type route: a draft item's title/body still go through one `item-edit` call; an issue's or pull request's go through the new content mutation; `Status` keeps its existing field-value route on all three, unchanged. The per-carrier refusal and its four consequences (editing-command refusal, settling-edit refusal, and the runner's take refusal) are retired — no bucket, terminal state, or verdict is removed, only the content-type **cause** that used to route into them. **This is a real behavior change, not just an unlock**: a body write against an issue- or PR-backed entry now edits that **live repository object** — visible to every viewer, entering its edit history, notifying its subscribers — which `/ptp:backlog-run`'s blast-radius announcement already disclosed and now actually does. `/ptp:backlog-run` can take a ready entry whatever its board item's content type. **Migration:** none required — no board change, no re-authentication beyond what 0.2.28 already asked for (a repository-scoped write may need a broader token depending on your `gh` auth setup; ptp discloses this via `contentScope` and never performs the credential refresh itself). (0048_01–0048_04) |
| **0.2.28** | **BREAKING — the backlog transport moves from the GitHub MCP server to the `gh` CLI, `backlog.mcpServer` is retired, and the `github-projects-mcp` capability is replaced by `github-projects-gh`.** The MCP path was not slow or awkward, it was **impossible**, and the contract had never been checked against a live server — its own text admitted the tool table was "unverified". Confirmed this release: no server exposes the eight discrete per-verb Projects tools the contract demanded; `ghcr.io/github/github-mcp-server` exposes three consolidated method-dispatch carriers, and its `projects_write` can only set a **project field**. It cannot create a board item (`add_project_item` requires an existing issue or PR and takes **no** title or body), cannot write any item's title or body, and the namespace ships **no issue-writing tool**, so "create an issue then link it" was unavailable too. The item **body** was not even readable — and it carries five of the ten entry slots. `--toolsets` controls *whether* Projects tools appear, never their shape, so no configuration change could have fixed this. `skills/ptp-github-projects-mcp` and the `github-projects-mcp` capability are **deleted rather than emulated**; `skills/ptp-github-projects-gh` and `github-projects-gh` replace them. **The preflight becomes four stages** — binary, authentication, **token scope**, board reachability — mapping gh's two Projects scopes onto the existing three verdicts with no fourth invented (`read:project` alone → `read-only`, `project` → `ready`, absent or logged-out → `unavailable`), and rendering the repair **verbatim as the CLI itself names it**. The preflight record drops `serverName`, `namespace`, `present` and `missing`, and gains the resolved project node id, which every field write needs as `--project-id`. **Configuration**: `backlog.mcpServer` is gone (`backlog.projectOwner` / `backlog.projectNumber` are unchanged); the acting identity is now **read from gh's own auth state and never chosen** — ptp sets no `GH_HOST`/`GH_TOKEN` and never runs `auth switch`/`login`/`refresh`. **Reading** is four calls — `project view`, `field-list`, `item-list`, and a `gh api graphql` supplement (same binary, same auth, same scope) that recovers the board stamps, `isArchived`, and the archived tier the porcelain silently omits, because `ProjectV2.items` defaults to `archivedStates: [NOT_ARCHIVED]`. That supplement is what binds **`archiveReachable` `true`**: degraded scope lifts, the ready set is **no longer withheld on every invocation**, and **`/ptp:backlog-run` can start again**. The body being readable on draft issues, issues and pull requests alike, the five body-carried slots return with **no body-unreachable mask**. gh's item `id` **is** the `ProjectV2Item` node id that `item-edit --id` accepts, so the node-id identity rule stands unqualified and the MCP path's node-id-vs-numeric handle split **dissolves**. **Writing** is restored end to end: `item-create --owner --title --body` creates a draft item carrying its **full composed body** — the single affordance whose absence made `/ptp:backlog-add` and `/ptp:backlog-run` impossible; `item-edit --id --title --body` updates both carriers in **one** mutation, and they are **co-dispatched by contract** because given only one flag gh refills the other from *its own* read, making a split write a silent lost update on the carrier holding four merge-written fields; the status commit resolves the option **id** from the field read. **The configuration gate becomes every command's own numbered first action**, ahead of `ptp-branch-guard` — it previously existed only as step 0 of skill pseudocode that no command's steps ever reached, so an invocation that provably could not write still stashed, pulled and cut a branch; and a board identity was rendered from values no configuration layer supplied. Every rendered board value now carries **per-value provenance**, an unsourced value has **no legal rendering**, and a server name is **never adopted from observation** — the contract's own example tables were being read as an adoptable menu. **Three new fatal conditions**, all on the existing problem codes with **no fifth code added**: a board field whose transport-flattened key collides with `id`, `content`, or the required carrier (gh writes those keys *before* its field loop, so the collision silently overwrites an entry's identity); more than 100 fields (gh pins `firstFields: 100` and pages only items); and an item returned with no interpretable content. Every list read now passes an **explicit limit reconciled against the response's own `totalCount`**, because gh's default of 30 would otherwise truncate a backlog into a short read. **Migration — read this if you use the backlog.** Run **`gh auth refresh -s project`** once (reading alone needs `read:project`); until then every backlog command refuses non-silently and names that repair. No board change is required and nothing on your board is rewritten. **One residual limit**, stated rather than hidden: an **issue- or PR-backed** entry's title and body have no in-board write path (that content lives outside the board and gh's title/body route accepts only a draft-content id), so those writes refuse per entry — its **`Status` writes still succeed**. (0047_02, 0047_05, 0047_06, 0047_07) |
| **0.2.27** | **BREAKING — the backlog board moves to five workflow columns, `Ready` becomes what "runnable" means, and `Todo` stops mapping to anything.** The entry `status` enum widens from five values to **seven** — `backlog`, `ready`, `in-progress`, `in-review`, `done`, `blocked`, `cancelled` — and the single `pending` value is **gone**, split into a **parked** status (`backlog`) and a **runnable** one (`ready`). The default option table gains `Backlog`, `Ready`, and `In Review`, and **drops `Todo` entirely**: it is deliberately *not* retained as an alias, because retaining it would make it impossible to stop `Todo` meaning `backlog` on a board that uses that column for something else. The **recommended board layout** is the five workflow columns in order — `Backlog` → `Ready` → `In Progress` → `In Review` → `Done` — with `Blocked` and `Cancelled` required as options ptp *writes* rather than columns a card is dragged through; that order is documentation, and ptp still **creates and reorders nothing** on the board. The **ready set is now the `ready` entries** (canonical `createdAt` order unchanged): an entry in `Backlog` is never taken, whatever its age, and an empty ready set alongside a full `Backlog` is a deliberate triage state raising no problem code. `/ptp:backlog-add` therefore creates entries at `backlog`, and promoting one is the new `backlog → ready` row of the transition table (with `ready → backlog` to defer again) — both unconditional and unguarded. A new **`in-review`** stage lands between convergence and archival: `/ptp:backlog-run` writes `in-progress → in-review` when `/ptp:full` converges and now writes `done` **nowhere at all**, which restores the invariant that the runner never marks work done that it did not archive and collapses `done` back to a single meaning — **archived**. `/ptp:backlog-continue` is the only command that reaches `done`, now from **either** `blocked` or `in-review` under guard 3, with `blocked` taking precedence in target selection and several `in-review` candidates resolving to the canonical-order head. The transition table grows to **eleven rows**, and prose everywhere cites a row by its **from → to pair, never by number**. `backlog.statusOptions` recognizes all seven status keys (an eighth is still ignored, never an error), and `/ptp:config`'s member menu for it grows from five rows to seven; the parameter count stays at **sixteen**. **Migration — read this if you have a board.** ptp changed nothing on your board and will change nothing: see **[Migrating a board built before this release](#migrating-a-board-built-before-this-release)** under one-time board setup for what still matches, what a `Todo` column now costs you, and the two remedies (rename/add columns, or configure `backlog.statusOptions`) — including the part a rename alone does not cover, that `Backlog` and `Ready` are a **workflow distinction** and migrated epics sit parked until moved to `Ready`. (0046_01–0046_04) |
| **0.2.26** | **The board's `Status` option names become configurable — `backlog.statusOptions`.** A board already running `Backlog / Doing / Shipped / Blocked / Dropped` is now usable as the epic backlog **without renaming a column**: the new key maps each of the five entry status values onto the board option name (or names) that carry it, at JSON path `["backlog","statusOptions"]` alongside the other `backlog.*` keys. **The existing fixed table is kept verbatim as the default**, so an unconfigured repository resolves exactly today's names for exactly today's statuses. Configuration merges **per status key** — a configured row **replaces** its default row, and an absent or invalid row falls back to **its own** default rather than discarding the whole map — and it changes *which* names are in the table, never *how* a name is matched (still case-insensitive and whitespace-trimmed) and never adds a sixth status. An option name outside the **resolved** table is still `malformed-entry` on `status`, **never coerced**. Two new refusals and one note: a resolved table in which one normalized name is claimed by **two** statuses is **unactionable**, refused non-silently at the configuration gate before any board call and never resolved by picking one; the write path now states, for the first time, **which** option a `status` commit writes — the **first name in the resolved row that the board actually offers**, so a README-shaped board gets `Todo` — and **refuses before the existence stage** when the board offers none of them (or two options matching the selected name), so a creation under that condition creates **no item**; and `/ptp:backlog` emits a **non-problem advisory** naming any status the board can carry no option for, which raises no code, withholds nothing, and changes no verdict. `/ptp:config` gains its first **map** parameter kind and its first **three-level** JSON path: after selecting `backlog.statusOptions` a member menu picks which of the five rows to set, and the row is typed as a comma-separated list (one name → JSON string, several → JSON array), with an empty element or a would-be collision rejected and re-prompted. The parameter set grows to **sixteen**. **No migration, no version-marker bump, no board mutation, no new problem code** — nothing stored changes, only the interpretation of names already on the board. Two limitations are documented rather than hidden: returning a row to its default requires deleting that key by hand, and an option name **containing a comma** must be hand-edited into the config file. (0045_02) |
| **0.2.25** | **BREAKING — the backlog entry's identifier becomes the board item's own node id, and the `Backlog ID` custom field is gone.** ptp no longer mints, writes, validates, or orders by a `BK-NNNN` identifier: the entry `id` is the item's opaque GraphQL **node id**, supplied by the transport on every item and never written by any writer. Everything that existed only to manufacture the old identifier is deleted rather than simplified — **id allocation** (`max + 1`, the zero-padding, the complete-fetch precondition for allocation, the reuse-after-hand-deletion limitation), the **membership rule** (so **every board item is an entry**, the *unmanaged item* concept, the trimming rule and the whitespace-lockout carve-out all go), the `duplicate-id` problem code and `malformed-entry`-on-`id` (both **unreachable by construction**, so the vocabulary drops **five codes → four**), and the write sequence's **identity stage** (existence → payload → commit, three stages; a creation is **two dispatches**). The carrier table drops **six → five** and the required-custom-field floor **two → one**: a board needs only `Status`. Ordering moves from ascending numeric id to the board's **`createdAt` ascending**, entries with an unusable stamp last and the node id as the final tie-break — the same FIFO semantic `/ptp:backlog-run` always consumed at the head of the ready set. **Writer eligibility** collapses to *refuses past any fatal problem, and past no structural problem at all*, so every structural defect is writer-eligible; **degraded scope** now withholds **only** the ready set, so `/ptp:backlog-add` proceeds under it and only `/ptp:backlog-run` refuses; and `archiveReachable`'s justification is re-grounded on the ready set's **head** rather than on id collision. **`/ptp:backlog-edit <node-id> "<what to change>"`** now takes the **first whitespace-delimited token** as its target, matched by exact node-id equality, refusing — and listing every entry's node id and title — when it matches none; `/ptp:backlog` renders that node id verbatim in its own copyable column. The out-of-enum `status` repair widens to **unset or out of enum**, so a hand-added card stays repairable through ptp. **Migration — there is none, by design.** Existing `Backlog ID` values simply stop being read; ptp never deletes the field, which becomes an ordinary unrecognized custom field, and you may remove it by hand. Two behavior changes to expect: a board with hand-added cards may order differently than before, and **a stray card with no `Status` now withholds the ready set** until it is set — the board is ptp's epic backlog, so do not park unrelated cards on it. (0045_01) |
