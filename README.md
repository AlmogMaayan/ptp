...
| Field | Type | Notes |
|---|---|---|
| `Status` | **single select** | Needs an option for each of the five status values. The names below are the **defaults**: `pending` (or `Todo`), `in-progress` (or `In Progress`), `done` (or `Done`), `blocked` (or `Blocked`), and `cancelled` (or `Cancelled` / `Canceled`). **A board whose columns are already named something else does not rename them** — set [`backlog.statusOptions`](#backlogstatusoptions) instead, per status row. |
...
ptp constant. `backlog.statusOptions` lets a board running `Backlog / Doing / Shipped / Blocked /
Dropped` be used as the epic backlog **without renaming a single column**.

**The key and its shape.** JSON path `["backlog","statusOptions"]`, sharing the same `backlog` parent as
the three keys above. Its value is an **object** whose recognized keys are the five entry status values
— `pending`, `in-progress`, `done`, `blocked`, `cancelled` — and whose value per key is either a
**string** or an **array of strings** naming the board `Status` option name(s) that carry that status:
...
A key outside those five is **ignored** — never an error, and it never adds a sixth status.

**The default is unset**, which means the built-in option table in full: `pending` from `pending` or
`Todo`; `in-progress` from `in-progress` or `In Progress`; `done` from `done` or `Done`; `blocked` from
`blocked` or `Blocked`; `cancelled` from `cancelled`, `Cancelled`, or `Canceled`. With the key unset,
every board item's status is interpreted exactly as it was before the key existed.

**Precedence is per status key.** Global first, then project — and within this key, **status key by
status key**. Configuring only `done` leaves the other four rows at their defaults. A configured row
**replaces** its default row rather than extending it, so configuring `pending: "Backlog"` means `Todo`
no longer reads as `pending` — which is the point: a board that uses `Todo` for something else needs to
be able to stop it meaning `pending`. If you want both spellings, list both.
...
- A layer that is **missing, unreadable, unparseable, has a non-object root, a non-object `backlog`, or
  a non-object `statusOptions`** supplies **no status key at all** — it is skipped for **all five**.
...
- **A colliding table refuses before any board call.** If one option name ends up on two rows of the
  resolved table — for example setting `pending: "Done"` while `done` keeps its default row —
...
- **A write refuses when your board carries no option for the status.** When ptp commits a status it
  ...
  defaults a board offering `Todo` and not `pending` gets `Todo`. If the board offers **none** of the
...
3. **Value** — select from the valid enum values with one-line descriptions (`codex.mode`,
   ...
   for the one **map** parameter (`backlog.statusOptions`), pick which of the five status rows to set
...
**`/ptp:backlog-add "<free-text description of the epic to add>" [model:<model>.<effort>]`** — adds **one** epic to the backlog from a free-text request: writes a single `pending` entry whose description keeps the request text substantively verbatim, and **modifies no other entry**. The entry's id is the board item's own node id — nothing is minted. The report names the entry it created, with its id, title, and status. Persistence runs `ptp-backlog-write`'s ordered write sequence — the create carrying title and body, then the `status: pending` commit last, **two dispatches** — with both re-reads and a full write journal, so a partial failure is inert, exactly reported, and never compensated. It carries **exactly one** up-front refusal, naming its own cause under an unusable backlog store (an incomplete configuration, a preflight verdict that does not admit a write, or a writer-ineligible store). Autonomous — it asks no clarifying questions.
