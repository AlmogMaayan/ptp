...
argument-hint: "<change-id-or-request> [--workspace <path>] (the big change to split; XXXX_NN_<kebab-description> id of an existing monolithic plan, an XXXX_00_<slug> epic container id (container mode), or a short description)"
---
...
Interpret it three ways, tested in this order: if `$ARGUMENTS` (after the token strip above) is a bare epic container id (story path exactly `00`), run in **container mode** — read the container as input, never modify or delete it; otherwise, if it names an existing `openspec/changes/<id>/` folder, treat that as a **monolithic plan to re-cut** (and a candidate for deletion in step 4); otherwise treat it as a **fresh request** to plan as multiple slices from scratch (nothing to delete).

Epic allocation (beat 1): allocate one fresh epic for all slices per the `ptp-change-selector` skill (§4, epic allocation). In container mode allocate **no** epic: the slices join the container's epic per `ptp-change-selector` §4c ("Decomposing with a container").

## Branch safety (beat 1, first write-affecting step)

**Beat 1 runs entirely in the outer session, in this exact order: (1) parse and strip the `parallel:` token (see *Inputs*), (2) allocate the fresh epic per `ptp-change-selector` §4 — or, in **re-cut mode**, allocate **no** epic and derive the child id prefix from the parent per §4b, the branch guard then deriving its leaf from the parent's epic rather than from a new one; in **container mode**, allocate **no** epic, the branch guard's leaf being the container id itself per `ptp-branch-guard` case 1, (3) run the `ptp-branch-guard` preamble once, (4) run step 1's gather-and-gate including its guaranteed-abort "implementation already started" STOP.** Sub-step (1) is **skipped when an orchestrating command supplied a pre-resolved parallel posture** — the token was already parsed and stripped upstream, so there is none to find. Skipping it changes nothing else about beat 1's ordering: (2), (3), and (4) run exactly as written. The epic allocation precedes the guard because the guard derives a fresh-request branch name — leaf `epic-XXXX`, shape per `ptp-workspace` — **from** the allocated epic. **No main run of any kind — not the beat-2 subagent, not a beat-3 member — starts before beat 1 has completed.**
...
**Three modes, differing only in how slices are named and what step 4 preserves.** Container mode is tested **first**, before a size halt: a container is never planned, so it can never halt for size, and testing it first never hides a re-cut.

| Mode | Selected by | Slice ids |
|---|---|---|
| **Container mode** | a bare epic container id `XXXX_00_<slug>` | the container's epic, numbered consecutively from its next story number (`01` for a fresh container) per `ptp-change-selector` §4c; the container is read in place and never written, moved or deleted |
...
1. **Gather input — do not delete anything yet** (outer session, beat 1). This whole step stays outer because it carries a **guaranteed abort** (a guaranteed abort must never spawn a subagent, and must never fan out):
   - **Container mode:** read the container's `prompt.md`, `brainstorm.md` and `analysis.md` in place (`ptp-change-selector` §4c); the container id names a folder holding no `tasks.md`, so the "implementation already started" check below passes. Nothing in the container is modified.
...
2. **Decompose (autonomous brainstorm).** Invoke **`ptp-brainstorming`** (or **`superpowers:brainstorming`** when the skill-set directive names `tdd-plugin=superpowers`) via the Skill tool in **autonomous mode** (no clarifying questions — document assumptions instead, exactly as `/ptp:plan` does), focused on a single question: *what is the smallest set of coherent, independently-shippable changes that together cover this request?* Produce an ordered list where each slice has:
   - a sub-change id. In **fresh decomposition** mode: `XXXX_NN_<kebab-description>` — a single epic allocated via `ptp-change-selector` (§4, epic allocation), then two-digit zero-padded story, then kebab description (e.g. `0001_01_landing-page-list-bulk-export`, `0001_02_landing-page-bulk-import`, `0001_03_landing-page-server-side-import`). In **re-cut** mode: a child of the parent per §4b — `0001_03_01_bulk-load-seam-read`, `0001_03_02_bulk-load-seam-write`. In **container mode**: `XXXX_NN_<kebab-description>` under the container id's epic, numbered consecutively from that epic's next story number per §4c (`0091_01_…`, `0091_02_…` for a fresh container `0091_00_…`). Either way the trailing story segment is the recommended apply order, and all slices share the same epic.
   ...
   **Dependency edges are verified against the code** — constructor injection, call sites, registration — never inferred from topic similarity. Slices that look independent by topic have been found mutually dependent through a single constructor and one nested call site.
   ...
   Do **not** persist this decomposition as its own file. It is working reasoning; it gets recorded durably inside each slice's `proposal.md` in step 5 (cross-reference only — no umbrella doc). The fresh-split container's decompose capsule (step 4) is not that file: it holds no slice list.
...
3. **Decide: split or fall back.**
   ...
   - If it is really one unit → **fall back**: return `PLAN-MULTIPLE-FALLBACK` with the original id/request as its single payload line (in container mode, the container id itself, which `/ptp:plan` then plans as the epic's next story per §4c) and **stop beat 2 there — do not run step 4**. Do not create artificial slices. (The outer session then invokes a single `/ptp:plan` and reports that no split was needed — see *Beat 2's return* below.)

4. **Create the epic container (fresh split), preserve what the slices do not re-author, then delete the parent plan if one exists — split path only.** Reached **only** when step 3 decided to split; on the fallback path step 3 already returned, so nothing here runs and no container is created. The mode decides what happens (`ptp-change-selector` §4c, "Decomposing with a container"):
   - **Container mode** — move nothing and delete nothing; the container stays exactly as it was.
   - **Re-cut** — unchanged, per §4b: never create, write, move or delete the epic container. If step 1 found the parent `openspec/changes/<id>/` **and confirmed no implementation had started**, discard the artifacts the children re-author (`proposal.md`, `design.md`, `tasks.md`, spec deltas, `effort.md`) and **move, never delete,** every other artifact the parent holds — `analysis.md` and `brainstorm.md` — into the **first child's** folder, where it is that child's direct input. Record each move in the step-6 report.
   - **Fresh decomposition** — create the new epic's container `XXXX_00_<desc>` now, before any member starts; `<desc>` is the input folder's desc, or the ≤5-word summary of the request.
     - **a. Preserve first.** If step 1 found an existing `openspec/changes/<id>/` **and confirmed no implementation had started**, move each of its `prompt.md`, `brainstorm.md` and `analysis.md` into the new container. The artifacts the slices re-author — `proposal.md`, `design.md`, `tasks.md`, spec deltas, `effort.md` — are discarded, their content having been folded into the decomposition. Any other file the folder holds is **moved, never deleted,** into the **first slice's** folder, as before. Record every move in the step-6 report.
     - **b. Capsule.** When no `brainstorm.md` was moved into the container — including when there was no input folder — write this beat's decompose capsule there as the container's `brainstorm.md`, in the `ptp-brainstorming` shape and holding **no slice list**. This is the container's one write, made by beat 2 before any member starts.
   - **Then delete** (fresh decomposition and re-cut only) the parent `openspec/changes/<id>/` folder — now that it holds nothing unpreserved — because leaving a half-planned parent beside its slices is confusing, and in re-cut mode a parent coexisting with its children would break the story ordering `ptp-change-selector` §1 depends on. With no such folder there is nothing to delete.

   Never delete before its thinking has been captured in step 2 and every artifact above has been moved, and never delete a change whose implementation has already started (step 1 already stopped you in that case).
...
5. **Plan each slice — one member per slice, started by the outer session.**
   ...
   **a. Assert the four `ptp-parallel-fanout` safety conditions**, and where each is established. Their normative definitions live in the **`ptp-parallel-fanout`** skill — cite it, do not restate it:
   ...
   |---|---|
   | 1. Write sets provably disjoint by construction | Every member's **work product** is written only within its own **pre-allocated** `openspec/changes/<id>/` folder (ids fixed in beat 2 → no allocation race, no shared folder). Writes into the shared `ptp-telemetry` store through that store's enumerated concurrency-safe protocols are the contract's **closed exception** (per `0034_01`) and do **not** defeat this condition. The epic container is written at most once, by beat 2 (the fresh-split capsule or moved files, step 4), **before any member starts**; no member writes it — each is told it is read-only context. No other shared path is written. |
   ...
   **d. The member prompt MUST carry**, verbatim in intent:
   ...
   - when the epic has a container (container mode, or the fresh-split container step 4 created), the **container id as read-only context**: *read its epic-level files if useful, treat its `brainstorm.md` as context only, and never write, move or delete anything in it* (`ptp-change-selector` §4c);
...
6. **STOP and report.** Do not start implementation. Emit the report **sorted by ascending change id**.
 ...
 Its content MUST be a function of the **joined member results** plus **beat 2's deterministic terminal metadata** (the slice list and the monolith-deletion outcome, both fixed before any member started) and MUST NOT vary with member **completion** order — so a parallel run and a serial run over the same joined results produce the same report content. Report:
   ...
   - **In re-cut mode:** the parent id that was replaced, where each artifact it held was moved (`brainstorm.md`, `analysis.md`), and every sibling `proposal.md` whose dependency reference was repointed onto the last child. Silence here would hide an artifact relocation and a cross-slice edit. This is a success state, so it is reported separately from the unsuccessful members above — but the epic is not fully planned while one stands, so offer no `/ptp:apply` command.
   - Whether a monolithic plan was deleted (and which id).
   - **The epic container:** in container mode, the container id, named as untouched; on a fresh split, the new container id, what was moved into it, and whether its `brainstorm.md` is beat 2's capsule.
...
- Do **not** delete the monolithic `openspec/changes/<id>/` folder until its thinking has been folded into the decomposition (step 2 before step 4), and — in fresh decomposition — only after its `prompt.md`, `brainstorm.md` and `analysis.md` (if any) have been moved into the new epic container `XXXX_00_<desc>`; the rest (proposal/design/tasks/spec deltas) is discarded. A re-cut parent's files move per §4b instead, never into the container; in container mode nothing is moved or deleted.
...
- **Never delete a parent folder while it still holds an artifact its children do not re-author** — `brainstorm.md` and `analysis.md`, whose deletion would silently destroy the slice's recorded inputs. Move first, delete second (step 4).
...
- Do **not** create an umbrella decomposition doc. The rationale + dependencies live as cross-references inside each slice's `proposal.md` (step 5). The fresh-split container's capsule (step 4) is not an umbrella doc: it holds no slice list.
