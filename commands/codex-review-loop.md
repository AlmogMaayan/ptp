...
**Review-convergence marker:** at either terminal state the loop stamps `openspec/changes/<change-id>/reviews/code.json` per `ptp-review-loop`'s **## Review-convergence marker** and **## Code-marker fingerprint** sections — `kind: "code"`, `terminalState` `converged` (DONE) or `cap-reached` (ITERATION CAP REACHED), `gateState` `LOOP_DONE` or `LOOP_CAP` (a standalone loop has no two-phase gate), `reviewers: ["codex"]`, the iteration count, the effective `minSeverity`, a timestamp, and — when it can be computed — the content `fingerprint`. No `/ptp:status` column reads this marker; it exists so a later caller can prove this review's convergence still describes the current code.
