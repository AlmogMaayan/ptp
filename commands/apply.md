...
3. **Implement tasks in order**:
   ...
   - **Test-first.** Resolve the `tdd` config key through `ptp-workspace`'s forgiving layered read (default `advisory`). Under resolved `tdd: mandatory`, test-first — the failing test before the minimal implementation — is required for every task that touches executable code (a prose-only task follows the `ptp-test-driven-development` prose-contract exemption instead); under resolved `tdd: advisory` it applies where the task is testable, as before. Under `mandatory` you MUST load `ptp-test-driven-development` for any task touching executable code and never for a prose-only task, whose exemption already owns that boundary; under `advisory` loading it is a MAY, as before. `ptp-verification-before-completion` stays a MAY under both. This mirrors `agents/ptp-apply.md` step 3 so both paths state one protocol.
...
4. **Final verification**:
   - All tasks checked.
   - Project test/lint/type suites pass. When the workspace root is the ptp repo itself — detected by a `.claude-plugin/plugin.json` whose `name` is `ptp` — that project suite is `node scripts/ptp-test.js <change-id>`; a consuming project's generic test/lint path is unchanged.
...
5. **Write the `apply` stage record, then return** the terminal result. Do **not** archive. Do **not** commit (apply defers commit to deploy — this is reinforced here, matching `ptp-full-apply`'s instruction to its apply agent). Report status.
   ...
   - **The optional `tests` array.** The record MAY additionally carry the apply-only `tests` array — at most one entry per task, each in one of two closed shapes: an executed `{ task, test, red, green }` (`red` = `<command> -> failed: <reason>`, `green` = `<command> -> passed`) or an exempt `{ task, exempt: "prose contract", reader }`. Its strings are an audit trail, never re-run. Under resolved `tdd: mandatory`, a task touching executable code that produced neither a `red` nor a valid `exempt` entry MUST NOT be checked `[x]`: it is left unchecked and the run maps to `blocked` naming that task. Under `advisory`, entries are recorded when the tests ran but never gate on their absence. The `tests` evidence lives only in the record; the relay vocabulary and return path are unchanged and nothing gates on the array.
