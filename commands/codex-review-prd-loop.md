---
description: Loop Codex PRD review + inline fixes until zero open findings at all severities or configured iteration cap reached (default 5; reviews the epic PRD openspec/prds/<epic>-<slug>.md, not artifacts or code; requires codex CLI on PATH)
...
automatically until every finding at all severities in the epic PRD `openspec/prds/<epic>-<slug>.md` is
resolved or the configured iteration cap (default 5) is reached.
...
stamps the **epic-scoped** marker at `openspec/prds/reviews/<epic>-<slug>.json` (`terminalState`
converged / cap-reached, `kind: "prd"`, `reviewers: ["codex"]`), via the same atomic
write-temp-then-rename protocol — one marker per epic. The `openspec/prds/reviews/` subfolder is created
on demand.
...
- Do **not** regenerate the PRD via `/ptp:prd`. All PRD fixes are minimal targeted hand-edits to
  `openspec/prds/<epic>-<slug>.md` only. A missing-PRD Critical has nothing to edit and the iteration
  cap is the backstop.
