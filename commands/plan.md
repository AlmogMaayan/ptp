...
2. **Invoke the PTP planning skills** (`ptp-brainstorming` first if missing, then `ptp-writing-plans`) via the Skill tool. Never write OpenSpec artifacts from raw user input.
   - When the spawn-prompt skill-set directive names `tdd-plugin=superpowers`, invoke `superpowers:brainstorming` in place of `ptp-brainstorming`.
   - Likewise, when the directive names `tdd-plugin=superpowers`, invoke `superpowers:writing-plans` in place of `ptp-writing-plans`.
...
When the skill-set directive names `tdd-plugin=superpowers`, that first skill is `superpowers:brainstorming` rather than `ptp-brainstorming`;
and the second is `superpowers:writing-plans` rather than `ptp-writing-plans`, again under `tdd-plugin=superpowers`.
...
2. **Run the PTP planning skills in this order** — both MUST be invoked via the Skill tool:
   ...
   - **(b) `ptp-writing-plans`** — produces the step-by-step implementation plan that feeds `tasks.md`. Must cover: files to add/modify, data/contract changes, migration concerns, test plan, rollback plan. Same autonomous-mode rule: no clarifying questions, no mid-flow approval gates. When the skill-set directive names `tdd-plugin=superpowers`, invoke `superpowers:writing-plans` here instead. If it returns **`NEEDS SPLIT`** (its terminal state — the change does not fit the artifact contract's keyed budgets), do not continue to step 3: carry that state and its proposed division straight to step 6.

     **`superpowers-output-override`** (path-override): when the `tdd-plugin=superpowers` arm ran and Superpowers produced the implementation plan, override Superpowers' default output path — that plan output is consumed into `<workspace root>/openspec/changes/<change-id>/tasks.md`, never written to `docs/superpowers/specs/...` or `docs/plans/` (workspace root carried verbatim via `ptp-run-at-model` part (g); never re-derive it); do not `git commit`/`git add` and do not stop at a human approval gate — this runs autonomously and ptp reviews afterward.
...
- Do **not** write a `proposal.md` whose content was not first produced by `ptp-brainstorming` (or `superpowers:brainstorming` when the skill-set directive names `tdd-plugin=superpowers`, or pulled from an existing brainstorming design doc). If you skipped brainstorming, you are violating the spirit of ptp — go back and invoke it.
