...
**Model dispatch target.** `/ptp:review-brainstorm` runs this skill's work at the `brainstorm` family default target (`skills/ptp-run-at-model/references/family-default-target.md`; `opus.high` built in) via `ptp-run-at-model` (`skills/ptp-run-at-model/SKILL.md`), which owns the spawn-and-relay mechanics and requires its caller to supply the target. This names the target only; it restates none of that contract.
...
dispatch over this skill's own rubric — the `(kind = brainstorm, reviewer = codex)` dispatch
...
review kind `brainstorm` (family `brainstorm`; `gpt-6-astra` / `high` when nothing is configured). This is the **read-only reviewer invocation**,
...
protocol inline, retargeted to `brainstorm.md` and with **NO** `openspec validate` (a brainstorm
precedes any proposal/spec): read `brainstorm.md` and any cited context yourself, build one
self-contained prompt carrying the rubric below plus the full brainstorm text and cited excerpts,
and pipe it to `codex exec -s read-only` over stdin. A *main* phase carries no `codex.mode` gate
(that gates a *reviewer*), so this skill owns its own precondition: when the resolved dispatch is
`codex`, verify `codex --version` is on PATH before running the pass; if `codex` is absent, **STOP**
and tell the user to install `codex` or set `roles.main = claude` in `/ptp:config`, rather than
silently falling back to a Claude pass. This skill's description stays a "main-agent" pass in both
directions.
