...
- the single up-front `codex` precondition that covers both phases,
...
`codex --version` must succeed. **Both** phases need Codex — the plan phase's `review-plan-full` runs a Codex artifact-review loop, and the run phase's `review-full` runs a Codex code-review loop. If `codex` is missing → **STOP** immediately with an install message and do **no** work: do not invoke `ptp:plan-multiple`, do not launch the run workflow, no Superpowers-only fallback. One check covers the whole command; nothing in the run removes Codex, so it is not re-checked before the run phase.
...
3. **Plan-convergence gate (symmetric, and it also gates the run phase).** A slice's `review-plan-full` has three terminal outcomes: `BOTH PHASES DONE` (the only green state), `ITERATION CAP REACHED` (Phase 1 capped, Phase 2 never ran), or `PHASE 2 ITERATION CAP REACHED` (Phase 1 converged, Phase 2 capped). If a slice terminates in **anything other than `BOTH PHASES DONE`**, **STOP after that slice**: do not plan-review any later slice, **and do not enter the run phase at all**. Keying the gate on the single green state — not just on the Phase-2 cap — is deliberate: a Phase-1 cap is equally non-converged, and applying code from a plan that did not fully converge is exactly what this gate prevents. Report which slice did not converge and its terminal state (see *Terminal report*).
...
Enter Phase B **only if every** captured slice reached `BOTH PHASES DONE` in Phase A. Then run the `ptp-full-run` skill's flow with the captured slice ids treated as an **explicit, ordered id list** (the plan order *is* the apply/dependency order):
...
- **`codex` missing** → the install message only.
- **Plan-convergence STOP** (a slice did not reach `BOTH PHASES DONE`) → the slices created so far, in order, each with its one-line scope and per-slice plan-review terminal state (`BOTH PHASES DONE` / `ITERATION CAP REACHED` / `PHASE 2 ITERATION CAP REACHED`); state plainly that the run phase was **not** entered. Recommend fixing the non-converged slice's artifacts (return to `/ptp:plan` for a Phase-1 cap, or `/ptp:review-fix` for Phase-2 Codex findings) and re-running `/ptp:full` (or `/ptp:review-plan-full <id>` for just that slice). The earlier slices that already reached `BOTH PHASES DONE` can be run on their own with `/ptp:full-run <converged-ids…>` if the user prefers to proceed with them first.
- **Ran to completion / run-phase halt** → first the plan-phase summary (slices in order, each one-line scope, all `BOTH PHASES DONE`), then the `ptp-full-run` **three-bucket terminal report** exactly as that skill specifies — `processed` / `applied (review pending)` / `never-started`, the per-slice outcome table, the resume command for the unprocessed tail, and a `/ptp:archive <id>` recommendation per fully-processed slice (never auto-run). Defer the bucket math to `ptp-full-run`; do not restate it here.
...
- **Codex is required** — check `codex --version` once up front; STOP with no work if missing. No Superpowers-only fallback, in either phase.
- **The plan-convergence gate blocks the run phase.** Enter the run phase only if **every** slice reached `BOTH PHASES DONE`. If any slice ends in `ITERATION CAP REACHED` (Phase-1 cap) or `PHASE 2 ITERATION CAP REACHED` (Phase-2 cap), do not apply any code — STOP after the plan phase and report.
