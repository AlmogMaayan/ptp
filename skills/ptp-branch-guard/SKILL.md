...
`analyze`, `brainstorm`, `brainstorm-only`, `brainstorm-full`, `plan`, `plan-multiple`, `prd`, `prd-full`, `apply`, `effort`, `review-fix`, `review-loop`, `review-plan-loop`, `codex-review-loop`, `codex-review-plan-loop`, `codex-review-prd-loop`, `review-full`, `review-plan-full`, `review-brainstorm-full`, `review-prd-full`, `full`, `full-plan`, `full-apply`, `archive`, `archive-force`, `backlog-run`, `backlog-continue`. (`backlog-run` and `backlog-continue` earn their place here not by writing the board directly — like `backlog-add`/`backlog-edit` below, that write is outside the repo tree — but because each drives further ptp steps that do write working-tree files: `backlog-run` runs `/ptp:full` per entry, and `backlog-continue` runs a review-then-archive sequence.)
