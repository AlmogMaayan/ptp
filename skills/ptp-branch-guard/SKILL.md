...
`analyze`, `brainstorm`, `brainstorm-only`, `brainstorm-full`, `plan`, `plan-multiple`, `apply`, `effort`, `review-fix`, `review-loop`, `review-plan-loop`, `codex-review-loop`, `codex-review-plan-loop`, `review-full`, `review-plan-full`, `review-brainstorm-full`, `full`, `full-plan`, `full-apply`, `archive`, `archive-force`, `backlog-run`, `backlog-continue`, `backlog-add-multiple`, `backlog-draft-to-ready`. (`backlog-run` and `backlog-continue` earn their place here not by writing the board directly — like `backlog-add`/`backlog-edit` below, that write is outside the repo tree — but because each drives further ptp steps that do write working-tree files: `backlog-run` runs `/ptp:full` per entry, and `backlog-continue` runs a review-then-archive sequence. `backlog-add-multiple` and `backlog-draft-to-ready` earn their place for a distinct reason again: each of their N written entries, like `backlog-add`'s, writes only the board and no repository file, but each command drives N sequential write operations — one per segmented description, or one per enumerated `backlog` entry — off a single main run rather than `backlog-add`'s or `backlog-edit`'s one operation, and runs the guard once in the outer session, defensively, before that multi-operation main run starts.)
