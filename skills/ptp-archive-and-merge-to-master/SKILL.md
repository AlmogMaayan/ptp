...
- **Ship summary** (merge `completed`) — the changes archived in order, then the ship summary (PR
  merged, branch deleted, deploy phase skipped by design (no run id), clean base branch).
- **Blocking-change STOP** (Phase A did not converge) — name the blocking change and the
  ...
  plainly that the merge phase was **not** entered (no partial merge) and list the
