---
name: ptp-archive-and-merge-to-master
description: Own the combined archive-then-merge-only flow, ordering the archive step ahead of the merge-to-master step
---

# ptp-archive-and-merge-to-master — archive-then-merge-only end-to-end orchestration

## Purpose

This skill is the orchestration contract behind the single `/ptp:archive-and-merge-to-master` command. It chains
two phases with a hard gate between them — **Phase A** archives each resolved change through the
existing `/ptp:archive` flow, an **archive-convergence gate** admits the merge only if *every* change
archived, and **Phase B** runs a **single** merge-only ship (`ptp-deploy` in `merge-only` mode, as
`/ptp:merge-to-master` drives it) on the current feature branch. The whole
reason the command exists is the ordering-plus-gate: archive **must** precede the merge (the merge
squash-merges the branch that must already contain the archive moves), and the merge must run **only
if all** archives succeeded (a partial archive followed by a merge would ship an inconsistent spec
state).
...
`sonnet.medium`); Phase B **is** the `ptp-deploy` skill's flow (driven exactly as `/ptp:merge-to-master`
drives it). This skill owns only the **glue and the archive-convergence gate between the two phases** —
...
1. **Outer session (interactive, before the spawn — the subagent is non-interactive per
   ...
   **or** `npx -y openspec validate <c> --strict` fails, **STOP the whole command here in the outer
   ...
   the blocking change and the gate it failed; do **not** merge). Only once tasks-complete and
...
3. **Relay `c`'s terminal state:** `completed` → continue to the next change; `refused` → record `c` as
   the blocking change and **STOP the whole command** (see the gate) — no merge.

## Archive-convergence gate

Phase B (merge) runs **only if every** resolved change reached the `completed` archive terminal state
...
The **first** such failure STOPs the command before the merge phase, and the terminal report **names
the blocking change and the gate/confirmation it failed**. There is **no** partial merge after a
...
## Phase B — merge (the ptp-deploy merge-only flow), exactly once, only on convergence

After **full** archive convergence (every resolved change `completed`), run the merge **exactly once**
on the **current feature branch** — the merge operates on the branch, **not** per change, regardless of
how many changes were archived. Reuse `ptp-deploy` via `ptp-run-at-model` at `sonnet.medium` (start
phase `commit`, mode `merge-only`), exactly as `/ptp:merge-to-master` does; do not restate
`ptp-deploy`'s methodology.

The merge runs **inline at one nesting level**. Because nesting is one level only (`ptp-run-at-model`
*Nesting caveat*), the wrapped `ptp-deploy` resolves its PR-stage bounded fix loop **inline** with the
subagent's own tools and **never spawns a nested fix subagent**. The orchestration skill itself runs in
the **outer** session and adds **no** extra nesting level: Phase A is one blocking subagent per change;
Phase B is one blocking subagent total.

Relay the merge's terminal state verbatim in meaning:

- `completed` → ship summary (PR merged, branch deleted, deploy phase skipped by design (no run id),
  clean base branch).
...
- `needs-human-action` → branch protection required an approving review that was not present; surface
  the reason, the PR URL, and the exact follow-up "re-run `/ptp:merge-to-master`". **Never
...
- **Ship summary** (merge `completed`) — the changes archived in order, the closing step's container
  lines (step 8: each checked container's target, or why it stayed or failed), then the ship summary
  (PR merged, branch deleted, deploy phase skipped by design (no run id), clean base branch).
- **Blocking-change STOP** (Phase A did not converge) — name the blocking change and the
  ...
  plainly that the merge phase was **not** entered (no partial merge), list the
...
- **Merge `refused`** — relayed as a gate/precondition failure (e.g. a fix-loop budget exhausted),
  **not** as success.
- **Merge `needs-human-action`** — the reason, the PR URL, and the exact follow-up "re-run
  `/ptp:merge-to-master`"; never reported as success.
...
- **Archive-convergence gate — no partial merge.** The merge runs only if **every** resolved change
  ...
  the whole command before the merge and names the blocker.
- **Merge runs exactly once** at the end, on the current branch (not per change).
- **Merge inline at one nesting level** — the wrapped `ptp-deploy` resolves its fix loops inline and
  never spawns a nested fix subagent.
- **Never self-approve / never `--admin`-bypass** a required approval — defer to `ptp-deploy`'s
  `needs-human-action` posture with the "re-run `/ptp:merge-to-master`" follow-up.
- **Never archive a half-set then merge** — the gate is all-or-nothing on archive.
