...
2. **Read each story's effort.** For each id, `Read` `openspec/changes/<id>/effort.md` and parse line 1 as `{model}.{effort}`. If the file is missing or line 1 is not a parseable `{model}.{effort}`, default to `opus.high` and **note the defaulting** (never crash). Then derive that story's **review target** from the same pair: `reviewModel` = `model` floored at `sonnet` (`haiku` → `sonnet`; `sonnet` and `opus` unchanged) and `reviewEffort` = `effort` floored at `high` (`low`/`medium` → `high`; `high` and `xhigh` unchanged), so the defaulted `opus.high` yields `opus` / `high` for the review too. This yields `{ id, model, effort, reviewModel, reviewEffort }` per story.
   ...
   **The `models.apply-review` entry (resolved once per run, beside `roles.main` and `tdd-plugin`).** Resolve `models.apply-review` through the forgiving layered configuration reader, accepting a layer's value only when it matches the `model:` token grammar per `skills/ptp-run-at-model/references/family-default-target.md` (never throw, never STOP). When the resolved entry is valid and its model is `haiku`, `sonnet` or `opus`, **every** story's `reviewModel` / `reviewEffort` is the entry's pair, replacing the derivation above — the `sonnet`/`high` floor does **not** apply to it, because an explicit review choice needs no lifting. When the entry names `fable` (outside the workflow's closed review vocabulary), keep each story's derived review target and **note in the run report that the entry was skipped**. When no layer supplies a valid entry, the derivation above is unchanged. The entry never changes a story's apply target (`model` / `effort`).

   **Per-story Codex dispatch targets (only under `roles.main=codex`).** Resolve `roles.main` **once** for the run through the forgiving layered configuration reader (`ptp-agent-roles` / `ptp-workspace` contract: missing/malformed/out-of-enum layer ignored, default `claude`, never STOP on a config typo). Under **`main=claude`** add **no** Codex fields — the `stories` array and every resulting prompt carry no Codex field (their review fields may differ only by a valid `models.apply-review` entry, above). Under **`main=codex`** resolve, per story, two Codex targets and add them to that story's entry as the optional fields `codexApplyModel`, `codexApplyEffort`, `codexReviewModel`, `codexReviewEffort`, **consumed by reference** (never restate the tier keys, the per-command-tier classification, or the Codex `effort.md`-line format):
   - the Codex **apply** target — resolved **exactly as** `ptp-run-at-model`'s `main==codex` branch resolves `/ptp:apply`: the story's own Codex `effort.md` line (`0076_03_codex-effort-rubric`'s line-2 location and parse rule) preferred, else that branch's fallback (`0076_02_run-at-model-codex-tier-consume`: its tier default, else flat `codex.model`/`codex.reasoningEffort`);
   ...
   A missing or unparseable `effort.md` still defaults the Claude line to `opus.high` and is noted, exactly as today; an unconfigured `codex` run (no tier keys, no Codex `effort.md` line) bottoms the apply target out at flat `codex.*` (which may themselves be unset — the workflow reads an empty value as absent and the agent falls back), while the review target still follows the `apply-review` chain. This yields `{ id, model, effort, reviewModel, reviewEffort }` per story under `main=claude`, plus the four `codex*` fields under `main=codex`.
...
- **A missing/unparseable `effort.md` defaults to `opus.high`** and is noted — never crash, never stop on it. The derived review target defaults with it (`reviewModel: 'opus'`, `reviewEffort: 'high'`), which is also what the workflow falls back to for an omitted or unrecognized value. A valid `models.apply-review` entry (step 2) replaces every story's review target instead; a `fable` entry is skipped with a note; resolving it never crashes or stops the run.
